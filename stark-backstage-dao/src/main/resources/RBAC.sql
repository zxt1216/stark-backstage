...
CREATE TABLE `STARK_ROLE` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '物理主键',
  `ROLE_ID` varchar(64)  NOT NULL COMMENT '业务主键(角色id)',
  `ROLE_NAME` varchar(18) NOT NULL COMMENT '角色姓名',
...
-- 部门表
DROP TABLE IF EXISTS `STARK_DEPT`;
CREATE TABLE `STARK_DEPT` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '物理主键',
  `DEPT_ID` varchar(64)  NOT NULL COMMENT '业务主键(部门id)',
  `DEPT_NAME` varchar(18) NOT NULL COMMENT '部门姓名',
  `DEPT_PARENT_ID` varchar(64) COMMENT '父级部门id',
  `DEPT_STATE` tinyint NOT NULL DEFAULT 0 COMMENT '部门状态:禁用1,启用0',
  `GTM_CREATE` datetime DEFAULT NULL COMMENT '创建时间',
  `GTM_UPDATE` datetime DEFAULT NULL COMMENT '修改时间',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `DEPT_ID_IDX` (`DEPT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
insert into STARK_DEPT(DEPT_ID,DEPT_NAME,GTM_CREATE,GTM_UPDATE) values('00000000000000000000000000000000','公司',now(),now());

-- 用户 部门关联表
DROP TABLE IF EXISTS `STARK_USER_DEPT`;
CREATE TABLE `STARK_USER_DEPT` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '物理主键',
  `USER_DEPT_ID` varchar(64)  NOT NULL COMMENT '业务主键(用户-部门id)',
  `USER_ID` varchar(18) NOT NULL COMMENT '用户id',
  `DEPT_ID` tinyint NOT NULL DEFAULT 0 COMMENT '部门id',
  `GTM_CREATE` datetime DEFAULT NULL COMMENT '创建时间',
  `GTM_UPDATE` datetime DEFAULT NULL COMMENT '修改时间',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `USER_DEPT_ID_IDX` (`USER_DEPT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- 资源表
DROP TABLE IF EXISTS `STARK_RESOURCE`;
CREATE TABLE `STARK_RESOURCE` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '物理主键',
  ...
  `RESOURCE_MENU_IDX` int(11) NOT NULL DEFAULT 0 COMMENT '节点排序号',
  `RESOURCE_URL` varchar(255) COMMENT '资源访问路径',
...
CREATE TABLE `STARK_ROLE_RESOURCE` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '物理主键',
...
CREATE TABLE `STARK_ACCESS_LOG` (
  ...
  `OPERATOR_NOTE` varchar(255) DEFAULT NULL COMMENT '备注,失败原因',
  `GTM_CREATE` datetime DEFAULT NULL COMMENT '创建时间',
  `GTM_UPDATE` datetime DEFAULT NULL COMMENT '修改时间',
  PRIMARY KEY (`ID`),
